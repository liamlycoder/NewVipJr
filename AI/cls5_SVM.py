...
X, y = make_blobs(n_samples=50, centers=2, random_state=0, cluster_std=0.6)   # 簇状数据
# X, y = make_circles(100, factor=0.1, noise=.1)  # 环形数据
# clf = SVC(kernel="linear")
clf = SVC(kernel="linear")
clf.fit(X, y)
print("得分: ", clf.score(X, y))
print("支持向量：", clf.support_vectors_)  # 返回支持向量
plot_svm(X, y, clf)
